...
#getwd()
#setwd("/Users/jozefmajsky/Desktop/Data Analytics with R")
...
load('./Data/trained_workflow.RData')
...
Race_analysis_content <- fluidPage(
  ...
  # Filters at the top
  fluidRow(
    column(6, pickerInput("driverNameInput1", "Select Driver", 
                          choices = sort(unique(df_joined$driverName)),
                          selected = NULL, multiple = TRUE, 
                          options = list(`actions-box` = TRUE))),
    column(6, pickerInput("yearInput1", "Select Year", 
...
server <- function(input, output, session) {
  ...
  prediction <- load('./Data/trained_workflow.RData')
