...
Race_analysis_content <- fluidPage(
  ...
  # Filters at the top
  fluidRow(
    column(6, pickerInput("analysis_driverNameInput", "Select Driver", 
                          choices = sort(unique(df_joined$driverName)),
                          selected = NULL, multiple = FALSE, 
                          options = list(`actions-box` = TRUE))),
    column(6, pickerInput("analysis_yearInput", "Select Year", 
...
page1Content <- fluidPage(
  ...
  fluidRow(
    column(6, pickerInput("records_driverNameInput", "Select Driver", 
                          choices = sort(unique(df_joined$driverName)),
                          selected = NULL, multiple = TRUE, 
                          options = list(`actions-box` = TRUE))),
    column(6, pickerInput("records_yearInput", "Select Year", 
                          choices = sort(unique(df_joined$year)),
                          selected = NULL, multiple = TRUE, 
                          options = list(`actions-box` = TRUE)))),
...
ui <- dashboardPage(
  dashboardHeader(
    title = tags$img(src = 'F1-Logo.png', style = 'height:30px; width:auto; margin:10px;')
  ),
  dashboardSidebar(
    sidebarMenu(
  ...
  dashboardBody(
    tags$head(
      ...
      #homeContent::before {
        ...
        right: 0;
        bottom: 0;
        background: url('F1-background.jpg') no-repeat center center fixed;
      ...
      #homeFooter {
        ...
        /* Custom padding for plots */
       .custom-plot-padding .plotly {
          padding-top: 50px; /* More space on the top */
          padding-bottom: 50px; /* More space on the bottom */
          padding-left: 30px; /* Adjust the left padding as needed */
          padding-right: 30px; /* Adjust the right padding as needed */
          box-shadow: 0 0 20px rgba(0,0,0,0.1); /* Optional: Adds a subtle shadow around the plot */
        }
        
        /* Custom CSS for the plot container */
      .plot-container {
        margin: 0 auto; /* Centers the plot */
      }
    ...
    tabItems(
      tabItem(tabName = "home", 
              div(id = "homeContent", 
                  ...
                  tags$img(src = 'F1-background.jpg', 
                           style = "position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: -1;"),
                  h1("Welcome to the F1 Race Analysis App!", 
                     style = "position: relative; color: white; z-index: 10;"),
                  div(id="homeFooter", 
                      "Master in Business Analytics - Final Project in Data Analytics with R - Group 5 Section B - 2024",
                      style = "position: absolute; bottom: 0; width: 100%; color: white; z-index: 10;")
              )
      ...
      tabItem(tabName = "cons_perf",
              fluidPage(
                ...
                div(class="custom-plot-padding"),
                plotlyOutput("teamPerformancePlot"),
                div(class = "footer",
                    tags$hr(),  # Horizontal line for separation
                    h5("Comment: This visualization provides insights into team performance across different years. Red Bull, Mercedes and Ferrari have been the top contenders in recent years.", 
                       style = "text-align: left; margin-top: 100px; padding-bottom: 10px;")  # Adjusted margin-top for spacing
                )
              )
      ),
      tabItem(tabName = "driver_perf",
              fluidPage(
                ...
                div(class = "footer",
                    tags$hr(),  # Horizontal line for separation
                    h5("Comment: This visualization shows the top 5 drivers based on total points each year. Top performers are stable.", 
                       style = "text-align: left; margin-top: 100px; padding-bottom: 10px;")  # Adjusted margin-top for spacing
                )
              )
      ),
      tabItem(tabName = "driver_circuit",
              fluidPage(
                ...
                div(class = "footer",
                    tags$hr(),  # Horizontal line for separation
                    h5("Comment: Maybe lucky field does exist??? :)", 
                       style = "text-align: left; margin-top: 100px; padding-bottom: 10px;")  # Adjusted margin-top for spacing
                )
              )
      ),
      tabItem(tabName = "wins_by_team",
              fluidPage(
                ...
                div(class = "footer",
                    tags$hr(),  # Horizontal line for separation
                    h5("Comment: Mercedes and Redbull it is (better bet on team than drivers?) :)", 
                       style = "text-align: left; margin-top: 100px; padding-bottom: 10px;")  # Adjusted margin-top for spacing
                )
              )
      ),
...
server <- function(input, output, session) {
  ...
  ana_filtered_data<- reactive({
    df_joined %>%
      filter(driverName %in% input$analysis_driverNameInput)%>%
      filter(year %in% input$analysis_yearInput)
  })
  
  req_filtered_data<- reactive({
    df_joined %>%
      filter(driverName %in% input$records_driverNameInput)%>%
      filter(year %in% input$records_yearInput)
  })
  ...
  output$driversTable <- renderDT({
    req(req_filtered_data())  # Ensure the filtered data exists
    #filter columns of table 
    data <- req_filtered_data() %>%
  ...
  output$teamPerformancePlot <- renderPlotly({
    p <- ggplot(performance_data, aes(x = year, y = total_points, group = name, color = name, text = paste("Team:", name, "<br>Total Points:", total_points))) +
      ...
      theme(
        ...
        plot.title = element_text(size = 16),  # Corrected to size for text elements
    ...
    ggplotly(p, tooltip = "text") %>%
      layout(hovermode = "closest",
             #Set the aspect ratio and size of the plot
             autosize = TRUE,
             width = 1100,  # Adjust the width 
             height = 500)   # Adjust the height  # Configure hover behavior
  })
  ...
  output$driverStandingsBarChart <- renderPlotly({
    ...
    plot <- ggplot(driver_standings_data, aes(x = reorder(driverRef, -total_points), y = total_points, fill = driverRef)) +
      ...
      labs(title = "", x = "", y = "Total Points") +
      theme_minimal() +
      theme(
        text = element_text(size = 10),
        axis.title = element_text(size = 11, face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1, face = "bold"),
        strip.text = element_text(size = 11, face = "bold")  # Style the facet labels
      ) +
    ...
    ggplotly(plot, tooltip = c("x", "y")) %>%
      layout(hovermode = "closest", 
             hoverlabel = list(bgcolor = "white", font = list(size = 16)),
             autosize = FALSE,
             width = 1100,  # Adjust the width 
             height = 500)
  })
  ...
  output$winHeatmap <- renderPlotly({
    ...
    plot_ly(long_data, x = ~Circuit, y = ~surname, z = ~Wins, type = 'heatmap', colorscale = 'Viridis') %>%
      layout(
        ...
        width = 1100,  # Adjust the width 
        height = 500# Adjust margins to fit axis labels
      )
  })
  ...
  output$raceWinsPieChart <- renderPlotly({
    ...
    plot_ly(data, labels = ~name, values = ~wins, type = 'pie',
      ...
      layout(showlegend = TRUE,
             ...
             margin = list(l = 50, r = 50, t = 50, b = 100),  # Adjust margins to give more space
             autosize = FALSE,
             width = 1100,  # Adjust the width 
             height = 500)
  })
  ...
  output$scatterPlot <- renderPlot({
    req(ana_filtered_data())  # Ensure data is ready
    data <- ana_filtered_data()
    if(nrow(data) > 0) {
